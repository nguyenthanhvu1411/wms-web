...
const PaymentRequestListPage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (code: string) => {
      const res = await inboundApi.getThreeWayMatches({ search: code });
      const match = res.items?.find((m: any) => m.matchNumber === code);
      if (!match) throw new Error(`Không tìm thấy đối chiếu có mã: ${code}`);
      return inboundApi.createPaymentRequest(match.id);
    },
    onSuccess: () => {
      toast.success('Tạo lệnh chi thành công');
      setIsModalOpen(false);
      setMatchCode('');
      queryClient.invalidateQueries({ queryKey: ['paymentRequests'] });
    },
    onError: (error: any) => {
      toast.error(error?.message || 'Lỗi khi tạo lệnh chi');
    }
  });

  const handleCreate = () => {
    if (!matchCode) {
      toast.error('Vui lòng nhập Mã Đối Chiếu');
      return;
    }
    createMutation.mutate(matchCode.trim());
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="flex justify-between items-center p-4 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-800">Tạo Lệnh Chi</h2>
            ...
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Mã Đối Chiếu (VD: M-...)</label>
                <input
                  type="text"
                  ...
                  placeholder="Nhập mã đối chiếu cần thanh toán"
                />
              </div>
            </div>
